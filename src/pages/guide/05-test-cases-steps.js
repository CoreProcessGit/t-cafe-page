...
const Steps = () => (
  <article className="guide-article">
    ...
    <aside className="guide-callout">
      <strong>민감 정보 주의</strong>: T-CAFE는 Test Data 내용을 데이터베이스에 저장할 때 암호화하고, 가져올 때 복호화해서 UI에 보여줍니다. 즉 <strong>T-CAFE 화면에서는 마스킹이 적용되지 않고 입력한 그대로 plain text로 표시</strong>되며, 권한이 있는 사용자라면 누구나 그대로 볼 수 있습니다. 실제 운영 비밀번호·API 키·개인정보를 직접 적지 말고, 테스트 전용 더미 계정 정보를 사용하세요.
    </aside>
...
- 어제 만든 데이터가 있어야 함 ← 검증 불가`}</code></pre>
    ...
    <h3>암호화 저장</h3>
    <p>T-CAFE는 사전 조건과 테스트 스텝을 산업 표준 암호화로 저장합니다. 사용자가 별도로 설정할 필요는 없습니다.</p>
